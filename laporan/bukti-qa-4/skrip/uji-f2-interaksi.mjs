#!/usr/bin/env node
// QA-4 F2 (salinan metode QA-2 C2 yang terbukti) — INTERAKSI: di tiap halaman, KLIK setiap elemen interaktif yang tampak (tautan internal, tombol, tab/pil, filter,
...
console.log(`# QA-4 F2 — interaksi & ketik penuh — ${U} — ${new Date().toISOString()}`);
for (const p of ['/', '/tentang', '/struktur', '/program', '/galeri', '/kontak', '/berita', '/berita?kategori=hukum', `/berita/${slug}`, '/lacak', '/faq']) await ujiHalaman(`publik ${p}`, `${U}${p}`);
...
console.log(`\nRINGKASAN F2: ${total} pemeriksaan, ${gagal} gagal -> ${gagal === 0 ? 'LULUS' : 'GAGAL'}`);
process.exit(0);
