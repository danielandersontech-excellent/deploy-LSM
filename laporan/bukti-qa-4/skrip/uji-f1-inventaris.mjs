#!/usr/bin/env node
// QA-4 F1 (salinan metode QA-2 C1 yang terbukti) — INVENTARIS: semua halaman (publik + staf) × 6 identitas (tamu + 5 peran) × 3 lebar (375/768/1280).
...
const PUBLIK = ['/', '/tentang', '/struktur', '/struktur?tampilan=peta', '/struktur?wilayah=13', '/program', '/program?kategori=pengawasan-dana', '/galeri', '/kontak', '/berita', '/berita?kategori=investigasi', '/berita?kategori=podcash', '/berita?q=dana', `/berita/${slug}`, '/lacak', '/lacak?nomor=WRP-000001', '/faq', '/kebijakan-privasi', '/pedoman-komunitas', '/halaman-tidak-ada'];
...
console.log(`# QA-4 F1 — inventaris halaman × identitas × lebar — ${U} — ${new Date().toISOString()}`);
...
console.log(`\nRINGKASAN F1: ${sel} sel, ${gagal} gagal -> ${gagal === 0 ? 'LULUS' : 'GAGAL'}`);
process.exit(0);
