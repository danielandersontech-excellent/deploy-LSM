...
| 04 Situs publik | BLOKIR (belum mulai) | | prasyarat mati: disk C: penuh (255 MB) → `npm run build` gagal exit 134 (V8 fatal). Belum ada berkas Tahap 4 yang ditulis |
...
- **Gejala:** `npm run build` gagal (`Next.js build worker exited with code: 134`, "Fatal error in , line 0") — dua kali, termasuk dengan TEMP/TMP dialihkan ke D:. Sisa C: 255 MB (D: lega 775 GB). Bukti: `laporan/bukti-tahap-04/00-BLOKIR-disk-penuh.txt`.
