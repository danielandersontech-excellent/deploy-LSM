...
/** Satu lampiran beserta pengaduan_id-nya (untuk route penyaji lampiran terjaga). */
export async function ambilLampiranById(id) {
  const baris = await kueri(`SELECT id, pengaduan_id, nama_berkas, path, tipe_mime, ukuran, dibuat_pada FROM pengaduan_lampiran WHERE id = ? LIMIT 1`, [Number(id)]);
  return baris[0] ?? null;
}
