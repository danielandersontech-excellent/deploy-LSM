...
const KOLOM_DAFTAR = `a.id, a.judul, a.slug, a.ringkasan, a.gambar_utama, a.status, a.jumlah_dibaca,
  ...
  k.id AS kategori_id, k.nama AS kategori_nama, k.slug AS kategori_slug, k.ikon AS kategori_ikon,
...
/** Batas bawah terbit_pada (WIB) untuk filter "Rentang Waktu" daftar berita: '30' | '90' hari terakhir, 'tahun-ini'. */
function batasBawahRentang(rentang) {
  if (rentang === '30' || rentang === '90') {
    return waktuSekarang(new Date(Date.now() - Number(rentang) * 24 * 60 * 60 * 1000));
  }
  if (rentang === 'tahun-ini') return `${waktuSekarang().slice(0, 4)}-01-01 00:00:00`;
  return null;
}

/** Daftar artikel TERBIT untuk publik; filter kategori (slug), kata kunci, dan rentang waktu ('30'|'90'|'tahun-ini'). */
export async function ambilArtikelTerbit({ halaman = 1, perHalaman = 9, kategoriSlug = null, q = null, rentang = null } = {}) {
  ...
  if (q) { syarat.push(`(a.judul LIKE ? OR a.ringkasan LIKE ? OR a.isi LIKE ?)`); params.push(`%${q}%`, `%${q}%`, `%${q}%`); }
  const batasBawah = batasBawahRentang(rentang);
  if (batasBawah) { syarat.push(`a.terbit_pada >= ?`); params.push(batasBawah); }
...
/** Modul "Paling Banyak Dibaca" (daftar_berita_investigasi): artikel terbit urut jumlah_dibaca. */
export async function ambilArtikelPalingDibaca(batas = 5) {
  return kueri(
    `SELECT ${KOLOM_DAFTAR} ${GABUNG} WHERE a.status = 'terbit' ORDER BY a.jumlah_dibaca DESC, a.terbit_pada DESC LIMIT ?`,
    [Math.max(1, Math.min(10, Number(batas) || 5))],
  );
}
...
/**
 * Daftar kategori berita. Bawaan hanya yang AKTIF (RUN QA-4 A: kategori lama dinonaktifkan, bukan dihapus);
 * `semua: true` menyertakan yang nonaktif (dipakai bila artikel lama masih menunjuk ke sana).
 */
export async function ambilKategoriArtikel({ semua = false } = {}) {
  return kueri(`SELECT id, nama, slug, deskripsi, urutan, ikon, aktif FROM kategori_artikel ${semua ? '' : 'WHERE aktif = 1'} ORDER BY urutan, nama`);
}

/** Satu kategori berdasarkan id (penjaga route: kategori harus ada dan aktif). */
export async function ambilKategoriArtikelById(id) {
  const baris = await kueri(`SELECT id, nama, slug, ikon, aktif FROM kategori_artikel WHERE id = ? LIMIT 1`, [Number(id)]);
  return baris[0] ?? null;
}
...
export async function ambilArtikelStaf({ peran, userId, wilayahId = null, status = null, q = null, halaman = 1, perHalaman = 10 }) {
  ...
  if (q) { syarat.push(`(a.judul LIKE ? OR u.nama LIKE ?)`); params.push(`%${q}%`, `%${q}%`); }
...
export async function perbaruiArtikel(id, { judul, ringkasan = null, isi, gambarUtama = null, kategoriId, wilayahId = null, tag = null }) {
  return transaksi(async (koneksi) => {
    // Slug hanya mengikuti judul selama artikel BELUM PERNAH terbit (terbit_pada NULL);
    // setelah terbit slug dibekukan agar tautan yang sudah tersebar tidak putus.
    const lama = await kueri(`SELECT slug, terbit_pada FROM artikel WHERE id = ? FOR UPDATE`, [Number(id)], koneksi);
    if (!lama[0]) return 0;
    const slug = lama[0].terbit_pada ? lama[0].slug : await slugUnik(buatSlug(judul), id, koneksi);
    const hasil = await kueri(
