...
const KOLOM = `p.id, p.nama, p.jabatan, p.tingkat, p.kelompok, p.bagian, p.wilayah_id, w.nama AS wilayah_nama, w.induk_id, p.foto, p.deskripsi,
...
export async function buatPengurus({ nama, jabatan, tingkat, kelompok = null, bagian = null, wilayahId = null, foto = null, deskripsi = null, aktifSejak = null, urutan = 0, aktif = 1 }) {
  const hasil = await kueri(
    `INSERT INTO pengurus (nama, jabatan, tingkat, kelompok, bagian, wilayah_id, foto, deskripsi, aktif_sejak, urutan, aktif) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [nama, jabatan, tingkat, kelompok, bagian, wilayahId, foto, deskripsi, aktifSejak, Number(urutan) || 0, aktif ? 1 : 0],
  );
  return hasil.insertId;
}

export async function perbaruiPengurus(id, { nama, jabatan, tingkat, kelompok = null, bagian = null, wilayahId = null, foto = null, deskripsi = null, aktifSejak = null, urutan = 0, aktif = 1 }) {
  const hasil = await kueri(
    `UPDATE pengurus SET nama = ?, jabatan = ?, tingkat = ?, kelompok = ?, bagian = ?, wilayah_id = ?, foto = ?, deskripsi = ?, aktif_sejak = ?, urutan = ?, aktif = ? WHERE id = ?`,
    [nama, jabatan, tingkat, kelompok, bagian, wilayahId, foto, deskripsi, aktifSejak, Number(urutan) || 0, aktif ? 1 : 0, Number(id)],
  );
