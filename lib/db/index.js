...
// QA-4 F4 (TEMUAN): pool dulu disimpan di variabel modul (`let pool`). Di bawah custom server, modul ini
// dimuat oleh BEBERAPA bundel (server.js, route API, halaman) sehingga tiap bundel diam-diam membuat pool
// sendiri (masing-masing sampai 10 koneksi) dan tutupPool() dari server.js tidak pernah menyentuh pool yang
// dipakai route -> setiap redeploy meninggalkan "Aborted connection" di log MariaDB. Pool kini disimpan di
// globalThis (pola yang sama dengan instance Socket.io) agar SATU pool dipakai seluruh proses dan bisa
// ditutup rapi saat SIGTERM.
const KUNCI_POOL = Symbol.for('warkop.pool');

/** Pool tunggal (lazy) — dibuat saat pertama dipakai, dipakai ulang di seluruh proses (semua bundel). */
export function db() {
  let pool = globalThis[KUNCI_POOL];
  if (!pool) {
...
export async function tutupPool() {
  const pool = globalThis[KUNCI_POOL];
  if (pool) {
    globalThis[KUNCI_POOL] = undefined;
    await pool.end();
  }
}
