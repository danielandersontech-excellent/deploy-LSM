...
function validasiNilaiPengaturan(kunci, nilai) {
  ...
  switch (def.tipe) {
    ...
    case 'url': {
      // QA-3 D2/E: URL boleh KOSONG (tautan/ikon sosial media disembunyikan bila kosong).
      // Hanya https yang diterima: mencegah mixed content dan skema berbahaya (javascript:, data:).
      const t = s.trim();
      if (!t) return '';
      if (t.length > BATAS_TEKS) throw new GalatValidasiPengaturan(`'${def.label}' maksimal ${BATAS_TEKS} karakter`, 'TERLALU_PANJANG', kunci);
      if (!/^https:\/\/[^\s<>"']+$/i.test(t)) throw new GalatValidasiPengaturan(`'${def.label}' harus alamat lengkap yang diawali https:// (diberi: '${t.slice(0, 40)}')`, 'TIPE_URL', kunci);
      return t;
    }
    case 'teks_panjang': {
      const t = s.replace(/\r\n/g, '\n').trim();
