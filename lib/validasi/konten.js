...
import { STATUS_PROGRAM } from '../kategoriProgram.js';
import { kategoriGaleriValid } from '../kategoriGaleri.js';
import { kelompokValid, kelompokBerbagian, bagianValid, jenisWilayahKelompok } from '../kelompokPengurus.js';
...
export function validasiPengurus(body) {
  ...
  // QA-3 A1: kelompok WAJIB dan harus ada di susunan final (lib/kelompokPengurus.js). Sebelumnya opsional,
  // dan itulah sebabnya penyuntingan bisa membuat pengurus kehilangan kelompok lalu hilang dari bagan.
  const kelompokMentah = teks(body.kelompok, 40);
  const kelompok = kelompokMentah ? kelompokMentah : null;
  if (!kelompok) throw new GalatValidasiKonten('Kelompok wajib dipilih', 'KELOMPOK_WAJIB', 'kelompok');
  if (!kelompokValid(kelompok)) throw new GalatValidasiKonten('Kelompok tidak dikenal', 'KELOMPOK_TIDAK_SAH', 'kelompok');
  // QA-3 A2: bagian hanya untuk kelompok berbagian (Direktorat), dan di sana wajib.
  const bagianMentah = teks(body.bagian, 60);
  const bagian = kelompokBerbagian(kelompok) ? (bagianMentah || null) : null;
  if (kelompokBerbagian(kelompok)) {
    if (!bagian) throw new GalatValidasiKonten('Bagian direktorat wajib dipilih', 'BAGIAN_WAJIB', 'bagian');
    if (!bagianValid(bagian)) throw new GalatValidasiKonten('Bagian direktorat tidak dikenal', 'BAGIAN_TIDAK_SAH', 'bagian');
  }
  const wilayahId = idOpsional(body.wilayah_id ?? body.wilayahId, 'wilayah_id', 'Wilayah');
  // QA-3 A3: kelompok DPW memilih PROVINSI, Koordinator Daerah memilih KABUPATEN/KOTA.
  // Jenis wilayahnya diperiksa di route (butuh basis data); di sini cukup keharusan mengisi.
  const jenisWilayah = jenisWilayahKelompok(kelompok);
  if (jenisWilayah && !wilayahId) {
    throw new GalatValidasiKonten(jenisWilayah === 'provinsi' ? 'DPW wajib memilih provinsi' : 'Koordinator Daerah wajib memilih kabupaten/kota', 'WILAYAH_WAJIB', 'wilayah_id');
  }
  ...
  return {
    nama, jabatan, tingkat, kelompok, bagian, wilayahId, foto: jalurGambar(body.foto, 'foto'), deskripsi: teksPanjang(body.deskripsi, 2000) || null,
...
export function validasiProgram(body) {
  ...
  // QA-3 F: daftar kategori kini DINAMIS (tabel kategori_program), jadi keabsahan slug diperiksa di
  // route yang punya akses basis data (lib/validasi sengaja murni & sinkron). Di sini hanya bentuknya.
  // Bila pemilik memilih "Kategori Lainnya..." maka `kategori` kosong dan `kategori_baru` yang diisi;
  // route-lah yang membuat kategorinya lalu mengisi slug. Karena itu kosong DIIZINKAN di sini.
  const kategoriBaru = teks(body.kategori_baru ?? body.kategoriBaru, 60);
  const kategori = teks(body.kategori, 50);
  if (!kategoriBaru && !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(kategori)) throw new GalatValidasiKonten('Kategori program wajib dipilih', 'KATEGORI_TIDAK_SAH', 'kategori');
