...
/**
 * Slug URL dari judul lewat paket `slugify` (TAHAP-05 aturan wajib): huruf kecil, ASCII,
 * tanda hubung, karakter selain [a-z0-9-] dibuang, dipotong maks 120 tanpa tanda hubung buntung.
 * Keunikan (akhiran -2, -3, …) dijamin di lib/db (slugUnik); slug TIDAK berubah setelah terbit.
 */
export function buatSlug(teks, maks = 120) {
  const dasar = slugify(String(teks ?? ''), { lower: true, strict: true, locale: 'id', trim: true });
  return dasar.slice(0, maks).replace(/-+$/g, '');
}
